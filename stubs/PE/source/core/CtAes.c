...
BOOL AESDecrypt(IN PBYTE pEncryptedBuffer, IN SIZE_T sEncryptedBufferSize, IN PBYTE pAESKey, IN PBYTE pAESIV, OUT PBYTE* ppDecryptedBuffer) {
    AES256_CBC_ctx AES256Ctx = {0};

    if (!pEncryptedBuffer || !sEncryptedBufferSize || !pAESKey || !pAESIV || !ppDecryptedBuffer) {
        return FALSE;
    }

    if (!(*ppDecryptedBuffer = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sEncryptedBufferSize))) {
        return FALSE;
    }

    RtlSecureZeroMemory(&AES256Ctx, sizeof(AES256_ctx));
    AES256_CBC_init(&AES256Ctx, pAESKey, pAESIV);
    AES256_CBC_decrypt(&AES256Ctx, (sEncryptedBufferSize / 16), *ppDecryptedBuffer, pEncryptedBuffer);
    return TRUE;
}